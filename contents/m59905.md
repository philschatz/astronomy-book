...
It is an enormous step to go from the planets to the stars. For example, our Voyager 1 probe, which was launched in 1977, has now traveled farther from Earth than any other spacecraft. As this is written in 2016, Voyager 1 is 134 AU from the Sun.[<sup>1</sup>](#footnote1){: data-type="footnote-number" name="footnote-ref1"} The nearest star, however, is hundreds of thousands of AU from Earth. Even so, we can, in principle, survey distances to the stars using the same technique that a civil engineer employs to survey the distance to an inaccessible mountain or tree—the method of *triangulation*.
...
1.  [1](#footnote-ref1){: data-type="footnote-ref" name="footnote1"} To have some basis for comparison, the dwarf planet Pluto orbits at an average distance of 40 AU from the Sun, and the dwarf planet Eris is currently roughly 96 AU from the Sun.
2.  [2](#footnote-ref2){: data-type="footnote-ref" name="footnote2"} Proper motion (as discussed in [Analyzing Starlight](/m59889){: .target-chapter}*,* is the motion of a star across the sky (perpendicular to our line of sight.)
